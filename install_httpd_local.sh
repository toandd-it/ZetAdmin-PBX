...
sudo yum -y install gcc make gcc-c++ cpp git openssl-devel m4 autoconf automake vim net-tools sqlite-devel sqlite psmisc ncurses-devel libtermcap-devel newt-devel libxml2-devel libtiff-devel gtk2-devel libtool libuuid-devel subversion kernel-devel kernel-devel-$(uname -r) crontabs cronie-anacron libedit libedit-devel libsrtp libsrtp-devel
...
echo " API URL   : http://"$pbx_domain"/api/zetadmin_pbx/postback.php"
